(define (domain mine-world3) ; HOW DO I KEEP TRACK OF THE CURRENT LOC OF THE ROBOT ? 
 ...
 ; #############################################################################################
    ...
    (:action Pickup
        ...
        :precondition (and  (Robotloc RM ?x)
                             ...
                             (not (Holding RM)) ; changed to make it hold any item 
                             (not(isOre ?o))
                      )
        :effect (and  (Holdwhat RM ?o)(not (IsHoldablein ?x ?o)) (Holding RM) (assign (battery-used) 3))
    )

    (:action BreakRock
        :parameters (?x -cell ?o - holdable ) ; a holdable ore is passed in 
        :precondition (and  (Robotloc RM ?x)
                             (Holdwhat RM Hammer) ; holding the hammer 
                             (IsObstaclein ?x Rock)
                             (IsHoldablein ?x ?o)
                             (isOre ?o)
                             ) 
    ...
    (:action Battery_Recharge
        ...
        :precondition  (and (Robotloc RM ?x)  (Chargeron ?x))  ; removed last constraint on battery recharge only if more than 20                    
    ...
    (:action Extinguish
        ...
        :precondition  (and (Robotloc RM ?x)  
                        (IsConnected ?x ?y)
                        (isFire ?y) 
                        (Holdwhat RM Extinguisher ))  ; last condition is to reduce recharges                        
        :effect (not(isFire ?y))
